...
chomp(my $headerFileContents = <<'HEADERFILECONTENTS');
%%%{PBXAllText}%%%
HEADERFILECONTENTS
...
if (length($selectedText) && ($selectedText =~ /([_A-Za-z][_A-Za-z0-9]*\s*)+([\s\*]+)([_A-Za-z][_A-Za-z0-9]*);/))
{
	...
	$name = $3;
	$behavior = "(assign) ";
	if ((defined($asterisk) && length($asterisk) == 1) or ($type eq "id"))
	{
		$behavior = "(retain) ";
	}
	else
...
chomp(my $implementationFileContents = do {local $/; <SCRIPTFILE>});
close(SCRIPTFILE);
